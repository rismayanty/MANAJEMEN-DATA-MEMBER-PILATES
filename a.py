...
def tampilkan_semua_pengguna(users):
    ...
    for uid, data in users.items():
        table.add_row([uid, data["nama"], data["usia"], data["role"],data("paket","-"),data("durasi","-"),data("jadwal","-")])
...
def cari_lihat_jadwal_member(users, pilates_shain, jadwal_kelas):
    ...
    if ditemukan in pilates_shain:
        ...
        print(f"Jadwal: {pilates_shain[JADWAL_PILATES]['jadwal']}")
    else:
...
def tambah_pengguna_admin(users):
    ...
    try:
        ...
        for i, j in enumerate(jadwal_paket):
